...
int16_t Filter2ndOrder(int16_t NewValue, float *Ins, float *Outs,
                       const float *ACoeffs, const float *BCoeffs) {
  float Xn;
  float Acc;

  Xn = (float)NewValue;
  Acc = (BCoeffs[0] * Xn) + (BCoeffs[1] * Ins[0]) + (BCoeffs[2] * Ins[1])
      + (ACoeffs[0] * Outs[0]) + (ACoeffs[1] * Outs[1]);
  Ins[1] = Ins[0];
  Ins[0] = Xn;
  Outs[1] = Outs[0];
  Outs[0] = Acc;
...
int16_t Filter1stOrder(int16_t NewValue, float *Ins, float *Outs,
  ...
  Xn = (float)NewValue;
  acc = (BCoeffs[0] * Xn) + (BCoeffs[1] * Ins[0]) + (ACoeffs[0] * Outs[0]);
